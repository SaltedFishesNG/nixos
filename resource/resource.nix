{ pkgs, userName, ... }:
{
  systemd.user.tmpfiles.users.${userName}.rules = [
    ...
    "L+ %h/.config/niri/bg.png          - ${userName} users - ${./bg.png}"
    "L+ %h/.config/niri/config.kdl      - ${userName} users - ${./.niri.kdl}"
    "L+ %h/.config/niri/lock.png        - ${userName} users - ${./lock.png}"
    ...
    "C  %h/.local/share/fish/fish_history 0644 ${userName} users - ${./.fish_history}"
  ];
}
